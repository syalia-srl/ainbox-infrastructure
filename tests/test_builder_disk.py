...
from ainbox_builder.catalog import CATALOG, BASE_IMAGE_GB, DEP_GB
...
def test_estimate_sums_base_models_and_modality_deps():
    sel = {"llm": [{"alias": "qwen3-14b"}], "stt": [{"alias": "whisper-small"}]}
    est = estimate_image_gb(sel, CATALOG, BASE_IMAGE_GB, DEP_GB)
    expected = (BASE_IMAGE_GB + CATALOG["llm"]["qwen3-14b"]["gb"]
                + CATALOG["stt"]["whisper-small"]["gb"] + DEP_GB["stt"])
    assert est == pytest.approx(expected)


def test_estimate_no_deps_when_dep_map_omitted():
    sel = {"llm": [{"alias": "qwen3-14b"}], "stt": [{"alias": "whisper-small"}]}
